<?php 
	 ...
	 * 
	 */
	class PemesananCustomerNovember extends CI_Controller
	{
		...
		function __construct()
	  	...
	  	function index(){
	  		if($this->session->userdata('akses') == 2 && $this->session->userdata('masuk') == true){
		       ...
		       $x['datapesanan'] = $this->m_pemesanan->getPemesananCustomerNovember();
		       $this->load->view('v_header',$y);
		       $this->load->view('admin/v_sidebar');
		       $this->load->view('admin/v_pemesanan_customer_November',$x);
		    }
		    else{
		       redirect('Login');
		    }
	  	...
	  	function savepemesananCustomer(){
			...
			$uang = $this->input->post('uang');
	  		...
	  		echo $this->session->set_flashdata('msg','success');
	       	redirect('Admin/PemesananCustomerNovember');		  	
 	  	}
